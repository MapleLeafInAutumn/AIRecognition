...
def plot_one_box(x, im, color=(128, 128, 128), label=None, line_thickness=3):
    ...
    names2 = ['jing_ru_dao_cha_right_open', 'jing_ru_dao_cha_left_open', 'hu_gui_li_kai', 'hu_gui_jing_ru',
            'fu_gui_to_dao_cha_right_open', 'fu_gui_to_dao_cha_left_open'
            ]
    res = False
    if res:
        a1, a2, a3, a4 = int(x[0]), int(x[1]), int(x[2]), int(x[3])

        corp = im[a2:a4, a1:a3, :]
        corp_gray = cv2.cvtColor(corp, cv2.COLOR_BGR2GRAY)
        ret2, th2 = cv2.threshold(corp_gray, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
        th3 = cv2.adaptiveThreshold(corp_gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, 33, 2)
