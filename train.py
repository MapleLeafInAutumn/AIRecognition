    ...
    for epoch in range(start_epoch, epochs):   # epoch
        ...
        if RANK in [-1, 0]:
            ...
            if (not nosave) or (final_epoch and not evolve):  # if save
                ...
                # torch.save(ckpt,  "F:/python_code/3_23/weights/3_26/epoch_{}.pt".format(epoch))
                if best_fitness == fi:
...
def parse_opt(known=False):
    ...
    parser.add_argument('--epochs', type=int, default=1120)
...
def main(opt):
    ...
    if opt.resume and not wandb_run:
        ...
        # with open('D:/3_13/yolov5-5.x-annotations-main/yolov5-5.x-annotations-main/runs/train/exp5/opt.yaml') as f:
    ...
    # 否则使用超参进化算法(遗传算法) 求出最佳超参 再进行训练
        ...
        meta = {'lr0': (1, 1e-5, 1e-1),  # initial learning rate (SGD=1E-2, Adam=1E-3)
                ...
                'flipud': (1, 0.0, 0.5),  # image flip up-down (probability)
        ...
        再确定哪一种进化方式，从而进行进化
        """
        for _ in range(360):  # generations to evolve
