...
def run(weights='../weights/last.pt', img_size=(640, 640), batch_size=1, device='cpu',
...
def parse_opt():
    """
    weights: 要转换的权重文件pt地址 默认='../weights3_21/best3_21.pt'
    img-size: 输入模型的图片size=(height, width) 默认=[640, 640]
    batch-size: batch大小 默认=1
    device: 模型运行设备 cuda device, i.e. 0 or 0,1,2,3 or cpu 默认=cpu
    include: 要将pt文件转为什么格式 可以为单个原始也可以为list 默认=['torchscript', 'onnx', 'coreml']
    ...
    parser.add_argument('--weights', type=str, default='../weights/best3_24.pt', help='weights path')
